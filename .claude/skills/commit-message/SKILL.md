...
- Types: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`,
  `build`, `ci`, `revert`.
- Scope: the feature the change touches, as the history already does —
  `document`, `deposit`, `files`, `search`, `users`, `validation`, `menu`,
  `translate`, `routing`. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(deposit)!: ...`.
